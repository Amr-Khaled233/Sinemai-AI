    ...
    <div className="space-y-4">
      {inquiries.map((inquiry) => (
        <Card key={inquiry.id} title={inquiry.subject}>
          <p className="mb-3 flex flex-wrap gap-2 text-xs text-[rgb(var(--muted))]">
            <span>
          ...
          <p className="whitespace-pre-line text-sm leading-7 text-[rgb(var(--text))]/90">{inquiry.message}</p>
          <p className="mt-4 text-xs">
            <a href={`mailto:${inquiry.contactEmail}`} className="text-teal-400 hover:underline" dir="ltr">
              {inquiry.contactEmail}
            </a>
            {inquiry.contactPhone && (
              <span className="ms-3 text-[rgb(var(--muted))]" dir="ltr">
                {inquiry.contactPhone}
              </span>
            )}
          </p>
